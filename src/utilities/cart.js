const ajax = require('../services/axios');
const db = require('../../models')
const { OrderServices, OrderServiceDetails } = db;
const merchantService = require('../services/merchant-service');
const userCart = {
    get:async function(token){
        ajax.setConfig({app:'cart',token:token});
        let cartItems = await ajax.getRequest('/cart/');
    ...
    clear: async function(token,cartId){
        ajax.setConfig({app:'cart',token:token});
        let cartItems = await ajax.deleteRequest('/cart/delete/',{cartId:cartId});
        return cartItems;    
    }
}
